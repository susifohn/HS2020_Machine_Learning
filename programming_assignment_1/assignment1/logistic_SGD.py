...
def logistic_SGD(X, y,num_iter=100000, alpha=0.01):
    ...
    for i in range(num_iter):
        ...
        #choose a random Training set, means select a random row of X with corresponding y
        import random
        rand = random.randint(0, X.shape[0]-1)
        
        #Update rule according lecture for this randomly selected training set
        theta = theta - alpha * gradient_function(theta, X[rand], y[rand])
